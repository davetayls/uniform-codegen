...
interface GenCommandArguments {
  cwd?: string
  componentsRootPath: string
  typesOutputFile: string
}

export const genCommand: CommandModule<{}, GenCommandArguments> =
  {
    ...
    builder(yargs) {
      return yargs
        ...
        .positional('componentsRootPath', {
          describe:
            'Path to components directory with yaml files',
          type: 'string',
          default: '',
        })
    ...
    async handler(argv) {
      ...
      const componentWriters = components.map(
        basicComponentWriter
      )
